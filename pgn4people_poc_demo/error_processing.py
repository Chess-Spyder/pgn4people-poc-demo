...
def log_nonfatal_error(string):
    """
    Logs an error message for a nonfatal error
    """

    logging.warning(format_error_text(string))


def fatal_error_exit_without_traceback(string):
    """
    Logs an error message for a fatal error, and exit without traceback
    ...
    logging.critical(format_error_text(error_message))
    sys.exit(1)
